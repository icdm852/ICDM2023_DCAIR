...
class AbstractTrainer(metaclass=ABCMeta):
    ...
    def test(self,code):
        ...
        if self.args.mode=='test':
            best_model_path = os.path.join(self.args.test_model_path, 'models', 'best_acc_model.pth')
            best_model = torch.load(best_model_path).get('model_state_dict')
            print('self.args.test_model_path:',self.args.test_model_path)
        else:
