...
class LoggerService(object):
    def __init__(self, train_loggers=None, val_loggers=None,val_loggers_head=None, val_loggers_tail=None):
        self.train_loggers = train_loggers if train_loggers else []
        self.val_loggers = val_loggers if val_loggers else []
        self.val_loggers_head = val_loggers_head if val_loggers_head else []
        self.val_loggers_tail = val_loggers_tail if val_loggers_tail else []
    ...
    def complete(self, log_data):
        ...
        for logger in self.val_loggers:
            logger.complete(**log_data)
        for logger in self.val_loggers_head:
            logger.complete(**log_data)
        for logger in self.val_loggers_tail:
            logger.complete(**log_data)
    ...
    def log_val(self, log_data):
        for logger in self.val_loggers:
            logger.log(**log_data)
    def log_val_head(self, log_data):
        for logger in self.val_loggers_head:
            logger.log(**log_data)
    def log_val_tail(self, log_data):
        for logger in self.val_loggers_tail:
            logger.log(**log_data)
